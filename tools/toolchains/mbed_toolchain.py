...
class mbedToolchain(with_metaclass(ABCMeta, object)):
    ...
    def _compile_sources(self, resources, inc_dirs=None, exclude_dirs=None):
        ...
        for source in compilation_queue:
            ...
            (commands, need_update) = self.compile_command(source.path, object, inc_paths)
            full_compile_command_queue.append({
                'source': source,
                'object': object,
                'commands': commands,
                'work_dir': work_dir,
            ...
            if commands is not None and need_update:
                queue.append({
                    'source': source,
                    'object': object,
                    'commands': commands,
        ...
        if self.export_compile_commands:
            ...
            lines = []
            for item in full_compile_command_queue:
    ...
    def compile_command(self, source, object, includes):
        ...
        if ext == '.c' or ext == '.cpp' or ext == '.cc':
            ...
            need_update = (len(deps) == 0 or self.need_update(object, deps))
            if ext != '.c' or self.COMPILE_C_AS_CPP:
                return (self.compile_cpp(source, object, includes), need_update)
            else:
                return (self.compile_c(source, object, includes), need_update)
        elif ext == '.s':
            deps = [source]
            deps.append(join(self.build_dir, self.PROFILE_FILE_NAME + "-asm"))
            need_update = self.need_update(object, deps)
            return (self.assemble(source, object, includes), need_update)
        else:
            return (False, False)
